 ...
 */
function gridbox_amp_menu_toggle() {
	if ( gridbox_is_amp() ) {
 ...
 */
function gridbox_amp_menu_is_toggled() {
	if ( gridbox_is_amp() ) {
 ...
 */
function gridbox_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? gridbox_get_svg( 'collapse' ) : gridbox_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'gridbox' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'gridbox' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'gridbox' ) : esc_html__( 'Expand child menu', 'gridbox' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'gridbox_amp_menu_dropdown_toggles', 10, 4 );
